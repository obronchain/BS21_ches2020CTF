...
def run_attack(attack_dir,ntraces_a,models):
    ...
    for files in files_traces:
        ...
        for start in range(0,len(traces),batch_enc):
            ...
            for k,m in models.items():
                enc_graph.set_init_distribution(k,
                            m["lda"].predict_proba(traces[start:start+bs,m["poi"]]))
            ...
            for shift in range(4):
                for w in range(4):
                    n[f"n_{w}_{shift}"][i:i+bs] = (nonces[start:start+bs,w] >>
                            (shift*8))&0xff
            del enc_graph
    ...
    with open(file_key) as fp: full_key = int(fp.readline(),16)
    secret_key = {}
    for w in range(4):
        for shift in range(4):
            secret_key[f"k_{w}_{shift}"] = ((full_key >> (w*32) )) >> (shift*8) & 0xff 
